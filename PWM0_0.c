...
{	
	...
	// Configure the PB6 pin to use the alternate function (M0PWMO)
	...
	// Disable the Module 0 PWM Generator 0 block (PWMO 0) before
	// configuration by clearing the ENABLE bit (Bit 0) in the PWMOCTL register
	PWM0->_0_CTL &= ~0x01;

	// Configure the counter for the PWMO 0 block to
	...
	// Enable the PWMO 0 signal to be passed to the PB6 pin (M0PWMO)
...
{
	// Set the duty cycle by wrirint the COMPA field (Bits 15 to 0)
