...
{	
	...
	// Enable the PWM Module 1 by activating the
	...
	// Clear the PMC2 fields (Bits 11 to 8) in the PCTL register
	GPIOF->PCTL &= ~0x00000F00;

	// Configure the PF2 pin to operate as a Module 1 PWM6 pin (M1PWM6)
	// by writing 0x5 to the PMC2 fields (Bits 11 to 8) in the PCTL register
	// The Ux value is derived from Table 3-5 in the TM4C123G Microcontroller Datasheet
	...
	// initialization by clearing the ENABLE bit (Bit 0) in the PWM1CTL register
	PWM1->_3_CTL &= ~0x01;

	// Configure the counter for the PWM1_3 block to
	// use the Count-Down mode by clearing the MODE bit (Bit 1)
	// in the PWM3CTL register. The counter will count form the load value
...
{
	// Set the duty cycle by writing the COMPA field (Bits 15 to 0)
	// in the PWM3CMPA register. When the counter matches the value in this register,
	// the PWM signal will driven high
	PWM1->_3_CMPA = (duty_cycle -1);
}
